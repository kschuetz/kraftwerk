...
class Functions {
    ...
    static <R> Generator<Fn0<R>> generateFn0(Generator<R> result) {
        return legacyTap(result, (output, rs) -> () -> output.run(rs).getValue());
    }

    static <A, R> Generator<Fn1<A, R>> generateFn1(Cogenerator<A> param1,
                                                   Generator<R> result) {
        return legacyTap(result, (output, rs) -> a -> output.run(param1.apply(rs, a)).getValue());
    }
    ...
    static <A, B, R> Generator<Fn2<A, B, R>> generateFn2(Cogenerator<A> param1,
        ...
        return Generator.legacyTap(result,
