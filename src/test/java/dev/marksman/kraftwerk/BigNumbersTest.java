...
class BigNumbersTest {
    ...
    class BigIntegers {
        ...
        @Test
        void alwaysInRange() {
            assertAlwaysInRange(BigIntegerRange.from(BigInteger.ZERO).to(BigInteger.TEN), BigNumbers::generateBigInteger);
            assertAlwaysInRange(BigIntegerRange.from(BigInteger.ONE.negate()).to(BigInteger.ONE), BigNumbers::generateBigInteger);
            assertAlwaysInRange(BigIntegerRange.from(LONG_MIN_VALUE.subtract(BigInteger.ONE))
                    .to(LONG_MAX_VALUE.add(BigInteger.ONE)), BigNumbers::generateBigInteger);
            assertAlwaysInRange(BigIntegerRange.from(LONG_MIN_VALUE.subtract(LONG_MAX_VALUE))
                    .to(LONG_MIN_VALUE), BigNumbers::generateBigInteger);
            assertAlwaysInRange(BigIntegerRange.from(LONG_MAX_VALUE)
                    .to(LONG_MAX_VALUE.add(LONG_MAX_VALUE)), BigNumbers::generateBigInteger);
        }
    ...
    class BigDecimals {
        ...
        @Test
        void alwaysInRange() {
            assertAlwaysInRange(BigDecimalRange.from(BigDecimal.ZERO).to(BigDecimal.TEN), BigNumbers::generateBigDecimal);
            assertAlwaysInRange(BigDecimalRange.from(BigDecimal.ONE.negate()).to(BigDecimal.ONE), BigNumbers::generateBigDecimal);
            assertAlwaysInRange(BigDecimalRange.from(LONG_MIN_VALUE.subtract(BigDecimal.ONE))
                    .to(LONG_MAX_VALUE.add(BigDecimal.ONE)), BigNumbers::generateBigDecimal);
            assertAlwaysInRange(BigDecimalRange.from(LONG_MIN_VALUE.subtract(LONG_MAX_VALUE))
                    .to(LONG_MIN_VALUE), BigNumbers::generateBigDecimal);
            assertAlwaysInRange(BigDecimalRange.from(LONG_MAX_VALUE)
                    .to(LONG_MAX_VALUE.add(LONG_MAX_VALUE)), BigNumbers::generateBigDecimal);
        }
