...
class Choose {
    ...
    static <A> OldGenerator<A> frequency(FrequencyEntry<? extends A> first, FrequencyEntry<? extends A>... more) {
        return frequencyImpl(cons(first, asList(more)));
    }

    static <A> OldGenerator<A> frequency(Collection<FrequencyEntry<? extends A>> entries) {
        return frequencyImpl(entries);
    }

    @SuppressWarnings("unchecked")
    private static <A> OldGenerator<A> frequencyImpl(Iterable<FrequencyEntry<? extends A>> entries) {
